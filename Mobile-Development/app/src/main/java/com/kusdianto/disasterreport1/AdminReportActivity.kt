...
class AdminReportActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btn_user_send_report.setOnClickListener{
            val name : String = edt_nama_bencana_admin.text.toString()
            val keterangan : String = edt_keterangan_admin.text.toString()
            val image : String = setOf(img_upload_display).toString()
        ...
        if (!allPermissionsGranted()) {
            ActivityCompat.requestPermissions(
                this,
                UserReportActivity.REQUIRED_PERMISSIONS,
                UserReportActivity.REQUEST_CODE_PERMISSIONS
            )
        }

        camBtnADMIN.setOnClickListener { //event button listener for camera
            captureImage()
        }

        chooseBtnADMIN.setOnClickListener { //event button listener for choose iamge
            val galleryIntent = Intent(
                Intent.ACTION_PICK,
                MediaStore.Images.Media.EXTERNAL_CONTENT_URI
            )
            startActivityForResult(galleryIntent, UserReportActivity.REQUEST_PICK_PHOTO)
        }
    }
    companion object {
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == REQUEST_TAKE_PHOTO || requestCode == REQUEST_PICK_PHOTO) {
            if (data != null) {
                ...
                imageAnaliticsADMIN.setImageBitmap(BitmapFactory.decodeFile(mediaPath))
                cursor.close()
                postPath = mediaPath
            }
